...
import { Sparkles, Zap, Code2, Layers, ArrowRight, Github, BookOpen, Package } from 'lucide-react';
...
export default function HelisaLanding() {
  ...
  const features = [
    {
      ...
      description: "Colección curada de componentes UI diseñados para proyectos modernos"
    },
    {
      icon: <Zap className="w-6 h-6" />,
      title: "Tailwind 4 Native",
      description: "Optimizado para Tailwind CSS 4, aprovechando las últimas características"
    },
    {
      icon: <Code2 className="w-6 h-6" />,
      title: "React Ready",
      description: "Integración perfecta con React 18+ y hooks para una experiencia de desarrollo fluida"
    },
    {
  ...
  const codeExample = `npx helisa-ui add button
# o añade múltiples componentes
npx helisa-ui add card dialog tabs`;
      ...
      <section className="relative z-10 container mx-auto px-6 pt-20 pb-32">
        <div className={`max-w-4xl mx-auto text-center transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <div className="inline-flex items-center gap-2 bg-purple-500/20 border border-purple-500/30 rounded-full px-4 py-2 mb-8 backdrop-blur-sm">
            ...
            <span className="text-sm text-purple-300">Construido con shadcn registry</span>
          </div>
          ...
          <p className="text-xl text-slate-300 mb-12 max-w-2xl mx-auto">
            Una librería de componentes custom premium para React con Tailwind 4 y basada en Shadcn. 
            Rápida, flexible y lista para producción.
          </p>

          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-16">
            <button className="group flex items-center gap-2 bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 px-8 py-4 rounded-xl font-semibold text-lg transition-all hover:scale-105 hover:shadow-2xl hover:shadow-purple-500/50"
                onClick={() => router.push('/docs')}
            >
              Comenzar ahora
              <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
            </button>
            <button className="flex items-center gap-2 bg-white/10 hover:bg-white/20 backdrop-blur-sm px-8 py-4 rounded-xl font-semibold text-lg transition-all border border-white/20"
                onClick={() => router.push('/docs')}
            >
      ...
      <section className="relative z-10 container mx-auto px-6 py-32">
        <div className="max-w-4xl mx-auto text-center bg-gradient-to-r from-purple-900/50 to-pink-900/50 backdrop-blur-sm border border-purple-500/30 rounded-3xl p-12">
          ...
          <p className="text-xl text-slate-300 mb-8">
            Únete a developers que ya están construyendo experiencias increíbles con Helisa
          </p>
          <button className="group inline-flex items-center gap-2 bg-white text-slate-900 hover:bg-slate-100 px-8 py-4 rounded-xl font-semibold text-lg transition-all hover:scale-105 hover:shadow-2xl"
            onClick={() => router.push('/docs')}
          >
            Empezar gratis
            <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
