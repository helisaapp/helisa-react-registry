    ...
    <div className={cn('w-full space-y-4 my-4', className)}>
      {Array(fields)
        ...
        .map((_, i) => (
          <div
            key={`field-${i}`}
            className={cn('space-y-2', layout === 'horizontal' ? 'grid grid-cols-3 items-center gap-4 space-y-0' : '')}
          >
            {withLabels && (
              <Skeleton
                className={cn(
...
const SelectSkeleton = ({
  ...
  return (
    <div className={cn('w-full space-y-2', className)}>
      ...
      <Skeleton className={cn(getHeight(), 'w-full rounded-md bg-gray-100 dark:bg-gray-800')} />
      {withHelperText && <Skeleton className="h-3 w-32 mt-1 bg-gray-200 dark:bg-gray-700" />}
    </div>
  );
};
...
const TableSkeleton = ({
  ...
  return (
    <div className={cn('w-full space-y-4 container mx-auto p-4 my-4 rounded-xl border bg-card', className)}>
      {withHeaderFilter && (
      ...
      <div className="rounded-md border overflow-hidden">
          ...
          <div className="bg-gray-50 dark:bg-gray-800 px-3">
            <div className="flex items-center h-10">
              {columnsHeader.map((_, i) => (
                <Skeleton
                  key={`header-${i}`}
                  className={cn('h-6 mx-2 flex-1 bg-gray-200 dark:bg-gray-700', cellClassName)}
                />
              ))}
            </div>
          </div>
        ...
        <div className="px-3">
          {Array(rows)
            .fill(null)
            .map((_, rowIndex) => (
              <div key={`row-${rowIndex}`} className={cn(`flex items-center border-t h-10`, rowClassName)}>
                {columnsHeader.map((_, colIndex) => (
                  <Skeleton
                    key={`cell-${rowIndex}-${colIndex}`}
                    className={cn('h-2 mx-2 flex-1 bg-gray-100 dark:bg-gray-700', cellClassName)}
                  />
                ))}
              </div>
            ))}
