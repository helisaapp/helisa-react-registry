...
// Was imported on that way to avoid tsconfig.json path mapping issues.
import * as registryImport from "../registry/index.js";
const registry = (registryImport as any).default.registry;
...
export const Index: Record<string, any> = {`;
  ...
  for (const item of registry.items) {
    ...
    const componentPath = item.files?.[0]?.path
      ? `@/registry/helisa/${item.files[0].path}`
      : "";
  ...
  "${item.name}": {
    ...
    files: [${item.files?.map((file: RegistryFile) => {
      const filePath = `registry/helisa/${typeof file === "string" ? file : file.path}`;
  ...
  const fixedRegistry = {
    ...
    items: registry.items.map((item: RegistryItem) => {
      const files = item.files?.map((file: RegistryFile) => {
        return {
          ...file,
          path: `registry/helisa/${file.path}`,
        };
      });
